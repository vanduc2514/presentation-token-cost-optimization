...
const customCss = `
  <style>
    ...
    @media (max-width: 900px) {
      ...
      .impact-box { font-size: clamp(1rem, 2.4vmin, 1.2rem); padding: 0.5rem 0.8rem; }
    }
    ...
    #step-10, #step-11, #step-12, #step-13, #step-14, #step-15, #step-16, #step-17, #step-18, #step-19, #step-20, #step-21, #step-22, #step-23, #step-24, #step-25, #step-26 { --group-accent: #a855f7; }
    #step-27 { --group-accent: #f59e0b; }

    /* ── THANK YOU SLIDE ───────────────────────────────────────────────── */
    #step-27 { justify-content: center; }
    #step-27 table {
      width: min(100%, 720px);
      margin: 1rem auto 0;
      border: 0;
    ...
    #step-27 tbody tr:nth-child(1) img {
      width: 180px;
      height: 180px;
      object-fit: contain;
      display: block;
    ...
    #step-27 tbody tr:nth-child(2) img {
      width: 36px;
      height: 36px;
      object-fit: contain;
      display: block;
...
(function() {
  ...
  /* Caveman skill image is inlined as base64 (class stripped by markpress) */
  var cavemanStep = document.querySelector('.step[data-y="-1800"]');
  if (cavemanStep) {
    cavemanStep.querySelectorAll('img').forEach(function (img) {
      img.addEventListener('click', function (e) {
        e.stopPropagation();
        openModal(img.src);
      });
